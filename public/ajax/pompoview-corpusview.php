...
<div id="<?php echo $currentid ?>-accordion">
	...
	<div>
		...
		<div class="pretty" id="<?php echo $currentid ?>-documentlist"></div>
		<button class="fat" onclick="PompOView.UI.vars('<?php echo $currenturl ; ?>').load();return false;">Valider ces options</button>
	</div>
...
<script type="text/javascript" charset="utf-8">
	...
	PompOView.UI.vars("<?php echo $currenturl ; ?>").getoptions = function () {
		...
		if ($('#<?php echo $currentid ; ?>-documentlist :checked').length > 1) {
			console.log("OK");
			options["corpus_sub"] = $('#<?php echo $currentid ; ?>-documentlist :checked').map(function() {return $(this).val();}).get();
		} else {
			console.log("Echec");
		};
	...
	PompOView.UI.vars("<?php echo $currenturl ; ?>").load = function () {
		...
		jQuery.post("<?php echo URL_AJAX; ?>fragment/pompoview-corpusview.documentlist.php",js,function (data) {
			$("#<?php echo $currentid ; ?>-documentlist").html(data);
		});
